...
EnsureSupportedSequenceColumnType(Oid sequenceOid)
{
	...
	sequenceOwned = sequenceIsOwned(sequenceOid, &tableId, &columnId);
	if (!sequenceOwned)
	{
		aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_SEQUENCE,
...
TypeOfColumn(Oid tableId, int16 columnId)
{
	...
	HeapTuple tup = NULL;
	Form_pg_attribute attrForm = NULL;
	...
	scanDescriptor = systable_beginscan(pgAttribute, AttributeRelidNumIndexId, true,
										NULL, scanKeyCount, scanKey);
